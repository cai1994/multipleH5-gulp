...
muiPull.prototype = {
    ...
    initPull: function(){
        ...
        that.$dom.append('<div class="noDataInfo"><div class="noDataInner"><img src="/common/noData/img/noData.png" /><p>暂无数据</p></div></div>');
            ...
            pullRefresh: {
                ...
                down: {
                    ...
                    callback : function(){
                        ...
                        //重置上拉状态
                        mui( '#'+ that.$list.attr('id') ).pullRefresh().disablePullupToRefresh();
                        mui( '#'+ that.$list.attr('id') ).pullRefresh().enablePullupToRefresh();
                        
                        //执行callback，callback需要有返回值，即数据给的data
                        that.callback && that.callback( $(this.wrapper.parentNode), 'down');
