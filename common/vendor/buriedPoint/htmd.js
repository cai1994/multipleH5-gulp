...
//_htmd方法为外部调用
//需传参tm-终端，pf-平台
//envOrigin--环境变量, 2--测试，3--预生产，4--生产
 ...
 //    	var tm = param.tm;
	...
	md.file.get = function( tm , pf){
		...
		if( pf == 1){
			...
			if( tm == 3 || tm == 2){
				...
				k.src = '/commonResources/common/vendor/buriedPoint/evt/path/H5PathList.js';
				var m = document.getElementsByTagName("script")[0];
				m.parentNode.insertBefore(k, m);
			}
			if( tm == 2){
				var k = document.createElement("script");
				k.src = '/commonResources/common/vendor/buriedPoint/evt/click/H5ClickList.js';
				var m = document.getElementsByTagName("script")[0];
				m.parentNode.insertBefore(k, m);
			}
		}
			...
			var k = document.createElement("script");
				k.src = '/commonResources/common/vendor/buriedPoint/evt/path/LgbPathList.js';
				var m = document.getElementsByTagName("script")[0];
				m.parentNode.insertBefore(k, m);
		}
	...
	(function(){
		...
		function commit( type , htmdEvt){
			...
			if( param.tm == 3 || param.tm == 2){
				...
				if( href.indexOf('/productPrivate/views/prdPrvDetails.html') != -1 ){
					//重设evt.info属性
					fData.evt.info = md.log.getQueryString("fundCode") + '_' + ( document.querySelector('.fundName').innerHTML ? document.querySelector('.fundName').innerHTML : '' );
				}
				//如果是公募产品详情页
					...
					fData.evt.info = md.log.getQueryString("fundCode") + '_' + inn;
				}
				//如果是现金宝详情页
				else if( href.indexOf('/cashTreasure/views/totalAssets.html') != -1 ){
					//重设evt.info属性
					fData.evt.info = md.log.getQueryString("fundCode") + '_' + (document.title ? document.title : '' );
				}
				//如果是注册页面
				else if( href.indexOf('/user/views/register.html') != -1 ){
					var redirectUrl = (md.log.getQueryString("redirectUrl") ? md.log.getQueryString("redirectUrl") : '');
					var trackModule = (md.log.getQueryString("trackModule") ? md.log.getQueryString("trackModule") : '');
					fData.evt.info = redirectUrl + '_' + trackModule;
				}
			}
		...
		if( window.mui){
			...
			$('body').on("click", '[htmdEvt]', function(event) {
				clickEvent(event);
			});
		}
		else{
