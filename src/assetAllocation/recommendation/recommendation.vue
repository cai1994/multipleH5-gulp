        ...
        <div class="mainCont">
            <div class="section">
                ...
                <div class="sec-content">
                    <div class="padB10">尊敬的{{customerData.customerName}}先生/女士：</div>
                    ...
                    <div class="indent2">如果您对《建议书》有任何问题或者有任何数据需要更新，欢迎您随时与恒天财富管理团队联系。我的电话是<span>{{customerData.phone}}</span>，电子邮件是<span>{{customerData.email}}</span>。 </div>
                    <div class="textR padT20">理财顾问：{{customerData.empName}}</div>
                    <div class="textR">报告日期：{{customerData.createTime}}</div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div v-html="content" class="ueditor"></div>
            </div>
            ...
            <div class="section">
                ...
                <div class="sec-content">
                    <div class="font-w">维度一：资产量级</div>
                    <div class="indent2">恒天财富将投资者的资产量级分为500万元以下、500-1000万元、1000-3000万元、3000万-1亿元、1亿元以上。</div>
                    <div>您的资产量级为：<span>{{customerData.canConfigAssetsToThousand}}</span>万元；属于{{customerData.assetScope}}</div>
                    <div class="font-w">维度二：生命周期</div>
                    <div class="indent2">恒天财富根据生命周期理论，将投资者的生命周期分为单身期、家庭形成期、家庭成长期、家庭成熟期和老年期。</div>
                    <div>您的生命周期处于：<span>{{customerData.lifeTermText}}</span></div>
                    <div class="font-w">维度三：风险偏好</div>
                    <div class="indent2">恒天财富根据投资者的风险承受能力，将投资者分为保守型、稳健型、平衡型、成长型、进取型。</div>
                    <div>您的投资类型为：<span>{{customerData.riskTypeText}}</span></div>
                    <div class="font-w">根据您的风险承受能力，建议您的资产配置如下：</div>
                    <div id="recommendPie" class="myChart">暂无数据</div>
                    <div class="">根据您的产品选择，建议您的产品配置如下：</div>
                </div>
                <ul class="dropDownList" ref="dropDownList">
                    <li class="item" v-for="(el1,index) in tableData">
                        <div class="header" @click="changeStatus(el1,url2)">
                            <p class="scale">
                        ...
                        <div class="fold" v-show="el1.isFold">
                            <div class="foldItem" v-for="(item,itemIndex) in el1.items">
            ...
            <div class="section">
                ...
                <ul class="dropDownList" ref="dropDownList">
                    <li class="item" v-for="(el2,index) in listData">
                        <div class="header" @click="changeStatus(el2,url1)">
                            <p class="scale">
                                <span class="orangeline"></span>
                        ...
                        <div class="fold" v-show="el2.isFold">
                            <div class="foldItem" v-for="(item,itemIndex) in el2.items">
                                <div class="title"><span class="size30 cl-3c3c3c">推荐</span><span class="cl-666"> {{item.productTypeText}}、 {{item.subdivisionStrategyText}}</span></div>
                                <ul class="foldItemList">
                                    <li class="listItem" v-for="(listItem,listItemIndex) in item.products">
                                        <div class="productName">
...
export default {
    ...
    data() {
        return {
            ...
            url1:apiUrl.selectAssetRecommendProductByAssetTypeApi,
            url2:apiUrl.selectAssetRecommendProductByCustomerApi,
            imgObj:{
                assetNowPieChart:'',
                assetRecommendPieChart:'',
                assetRecommendLineChart:'',
                assetConfigId:'',
            },
            isPdf:false
        };
    ...
    created: function() {  
        this.getDic(); 
        this.selectAssetInvestSuggestInfo();
    ...
    methods: {
        ...
        getDic(){
            this.$axiosHttp.http({
               ...
               /* 1000是与否
                ...
                this.getCustomerAssetConfigDetail();
            }, res => {
        ...
        getAssetTypeList(){
            this.$axiosHttp.http({
                ...
                this.listData = this.$utils.objDeepCopy(res.data.data);
                this.tableData = this.$utils.objDeepCopy(res.data.data);

                this.DicConversion(this.listData);
                this.DicConversion(this.tableData);
                console.log(this.listData==this.tableData)
        ...
        //资产配置报告详情
        getCustomerAssetConfigDetail(){
            this.$axiosHttp.http({
                name:'资产配置报告详情',
                url: apiUrl.getCustomerAssetConfigDetailApi,
                needFailCallback:true,
                params: {
                    id:splitUrl()['assetConfigId']
                }
                ...
                this.customerData.canConfigAssetsToThousand = this.$utils.toThousand(this.customerData.canConfigAssets);
                let canConfigAssets = this.customerData.canConfigAssetsToThousand;

                if (canConfigAssets<500) {
                    this.customerData.assetScope="500万元以下"

                }else if (canConfigAssets >= 500 && canConfigAssets<1000) {
                    this.customerData.assetScope="500-1000万元"

                }else if (canConfigAssets >= 1000 && canConfigAssets<3000) {
                    this.customerData.assetScope="1000-3000万元"

                }else if (canConfigAssets >= 3000 && canConfigAssets<10000) {
                    this.customerData.assetScope="3000万-1亿元"

                }else if (canConfigAssets >= 10000) {
                    this.customerData.assetScope="1亿元以上"

                }

                this.assetConfigAmountList=res.data.data.assetConfigAmountList;

                for (var i = 0; i < this.lifeTermDic.length; i++) {//生命周期阶段
                        var obj =this.lifeTermDic[i];
                        if(obj.keyNo==this.customerData.lifeTerm){
                            this.$Vue.set(this.customerData, 'lifeTermText', obj.keyValue);
                        } 
                    }

                for (var i = 0; i < this.riskTypeDic.length; i++) {//风险类型
                    var obj =this.riskTypeDic[i];

                    if(obj.keyNo==this.customerData.riskType){
                        this.$Vue.set(this.customerData, 'riskTypeText', obj.keyValue);
                    } 
                }
                ...
                this.assetConfigAmountList.forEach((course, courseIndex, array) =>{
                    ...
                    this.assetRecommendObj.titleText=this.customerData.assetScope+"+"+this.customerData.lifeTermText+"+"+this.customerData.riskTypeText
                ...
                if (this.assetNowObj.seriesData.length != 0) {
                    this.imgObj.assetNowPieChart=this.drawPie(this.assetNowObj);
                }else{
                    $("#nowPie").addClass("noChart");
                }
                if (this.assetRecommendObj.seriesData.length != 0) {
                    this.imgObj.assetRecommendPieChart=this.drawPie(this.assetRecommendObj);
                }else{
                    $("#recommendPie").addClass("noChart");
                }
                
                if (this.BarObj.data1 != 0 ||this.BarObj.data2 != 0) {
                    this.imgObj.assetRecommendLineChart=this.drawBar(this.BarObj);
                }
                
                

                this.imgObj.assetConfigId=splitUrl()['assetConfigId'];
        ...
        //资产投资建议查询
        selectAssetInvestSuggestInfo(){
            this.$axiosHttp.http({
                name:'资产投资建议查询',
                url: apiUrl.selectAssetInvestSuggestInfoApi,
        ...
        submitForm(){
            ...
            if (this.isPdf) {
                var assetConfigId=splitUrl()['assetConfigId'];
                var customerName=this.customerData.customerName;
                window.location.href=apiUrl.downloadPdfReportApi+'?assetConfigId=' + assetConfigId + '&customerName=' + encodeURI(customerName);
            }
        }
    }
};
