    ...
    <template>
      <ul class="tableList listTemp" id="tableList">
        <li class="topBottomDivide dataList" v-for="(item,index) in listData" :key="index">
          ...
          <p>
            <span class="person">{{item.custName}}({{item.custCode}})</span>
...
export default {
  ...
  methods: {
    ...
    goBack() {
      ...
      let url = "";
      url = `${goUrl.monthPERF}?year=${urlData.year}&month=${urlData.month}`;
    ...
    getData() {
      ...
      this.httpUrl = apiUrl.queryEmpPerformDetail;
